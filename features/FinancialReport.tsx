...
import { BanknotesIcon, TrendingDownIcon, WalletIcon, ChartBarIcon } from '../components/Icons';
...
export const FinancialReport = ({ fees = [], expenses = [], students = [] }: FinancialReportProps) => {
  ...
  const renderStatCards = (acquired: number, spent: number, balance: number) => (
    <div className="stat-grid" style={{ marginBottom: 24 }}>
      <div className="stat-card stat-forest">
        <div className="stat-icon"><BanknotesIcon /></div>
        <div>
          <p className="stat-label">Money Acquired</p>
          <p className="stat-value">GH₵ {acquired.toFixed(2)}</p>
        </div>
      </div>
      <div className="stat-card stat-red">
        <div className="stat-icon"><TrendingDownIcon /></div>
        <div>
          <p className="stat-label">Money Spent</p>
          <p className="stat-value">GH₵ {spent.toFixed(2)}</p>
        </div>
      </div>
  ...
  const renderTable = (data: Record<string, number>, label: string, colorClass: string) => (
          ...
          <tbody>
            {Object.entries(data).map(([key, amt]) => (
              <tr key={key}>
                ...
                <td className={colorClass} style={{ fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
                  GH₵ {(typeof amt === 'number' ? amt : 0).toFixed(2)}
                </td>
              </tr>
            ))}
          </tbody>
    ...
    ) : <p className="text-muted" style={{ padding: '16px 0' }}>No data recorded for this period.</p>
  );
    ...
    <div className="page-container">
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        <h2 className="card-title">
          <ChartBarIcon className="w-5 h-5 text-forest" />
          Generate Report
        </h2>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-end' }}>
          <div>
            ...
            <div className="toggle-group">
              ...
              <button className={`toggle-btn ${reportType === 'range' ? 'active' : ''}`} onClick={() => setReportType('range')}>Date Range</button>
            </div>
          </div>

          {reportType === 'daily' ? (
            <div>
              <label className="form-label" style={{ marginBottom: 6, display: 'block' }}>Report Date</label>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 20 }}>
                    ...
                    <tbody>
                      {Object.entries(dailyReport.studentFees).map(([student, payments]) => (
                        <tr key={student}>
                          ...
                          <td style={{ fontWeight: 700, color: 'var(--color-forest)', fontVariantNumeric: 'tabular-nums' }}>
                            GH₵ {payments.reduce((s, p) => s + p.amount, 0).toFixed(2)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className="card">
              ...
              {renderTable(dailyReport.expenseBreakdown, 'Category', 'fee-color-danger')}
            </div>
          </div>
        </>
      )}
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 20 }}>
              <div className="card">
                ...
                {renderTable(rangeReport.studentFees, 'Student', 'fee-color-success')}
              </div>
              <div className="card">
                <h2 className="card-title">Expenses by Category</h2>
                {renderTable(rangeReport.expenseBreakdown, 'Category', 'fee-color-danger')}
              </div>
            </div>
          </>
        ) : (
